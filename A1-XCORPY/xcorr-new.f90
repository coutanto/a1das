...
#ifdef MKL
!$OMP PARALLEL default(shared), private(l, Stats,cxt)
allocate(cxt(nfft))
...
do l=1,nx
    ...
    ! cshift can cause a stack overflow for large array, this is known and described in
    ! https://gcc.gnu.org/bugzilla/show_bug.cgi?id=12533
    !cxt = cshift(cxt,-lag)
    !xcorr(1:len_xcor,l) = cxt(1:len_xcor)
    write(0,*) 'nfft, len_xcor, lag',nfft, len_xcor, lag, nx, l

    xcorr(1:lag,l) = cxt(nfft-lag+1:nfft)
    write(0,*)lag+1,len_xcor,l,shape(xcorr)
    !xcorr(lag+1:len_xcor,l) = cxt(1:nfft-lag)
    do k=lag+1,len_xcorr
        xcorr(k,l) = cxt(k-lag)
    end do
    write(0,*) 'done'
end do
